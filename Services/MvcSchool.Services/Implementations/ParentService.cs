...
namespace MvcSchool.Services.Implementations
{
    public class ParentService : IParentService
    {
        ...
        public IEnumerable<ParentProfileFullServiceModel> GetAllParentProfilesFull(int page = 1)
        {
            var allParentProfilesFull = this.db
                .Parents
                .Skip((page - 1) * ParentsPageSize)
                .Take(ParentsPageSize)
                .Select(prnt => new ParentProfileFullServiceModel
            {
                Id = prnt.Id,
        ...
        public ParentProfileFullServiceModel GetParentProfileFullById(int id)
        {
            var parentProfileFullById = db.Parents.Where(x => x.Id == id).Select(prnt => new ParentProfileFullServiceModel
            {
                Id = prnt.Id,
                FirstName = prnt.FirstName,
                MiddleName = prnt.MiddleName,
