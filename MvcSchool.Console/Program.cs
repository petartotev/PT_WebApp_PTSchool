...
//var studentsInCouncil = db.Students.Where(x => x.IsSchoolCouncilMember == true).ToList();

//Select Name of Class of Student With Id = 555.
//var idOfClassOfThisStudent = db.Students.Where(x => x.Id == 555).FirstOrDefault().ClassId;
//var classOfThisStudent = db.Classes.Where(x => x.Id == idOfClassOfThisStudent).FirstOrDefault().Name;
//System.Console.WriteLine(classOfThisStudent);

//Select Parents of All Students from class 10A.
//var parentsOfStudentsFromClass10a = db.StudentsParents.Where(x => x.Student.Class.Name == "10a").Select(x => x.Parent).ToList();
//foreach (var item in parentsOfStudentsFromClass10a)
//{
